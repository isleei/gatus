...
func (a *API) createRouter(cfg *config.Config) *fiber.App {
	...
	protectedAPIRouter.Post("/v1/admin/reload", TriggerImmediateConfigurationReload(cfg))
	...
	protectedAPIRouter.Get("/v1/admin/suites", GetManagedSuites(cfg))
	protectedAPIRouter.Post("/v1/admin/suites", CreateManagedSuite(cfg))
	protectedAPIRouter.Put("/v1/admin/suites/:key", UpdateManagedSuite(cfg))
	protectedAPIRouter.Delete("/v1/admin/suites/:key", DeleteManagedSuite(cfg))
	protectedAPIRouter.Get("/v1/admin/external-endpoints", GetManagedExternalEndpoints(cfg))
	protectedAPIRouter.Post("/v1/admin/external-endpoints", CreateManagedExternalEndpoint(cfg))
	protectedAPIRouter.Put("/v1/admin/external-endpoints/:key", UpdateManagedExternalEndpoint(cfg))
	protectedAPIRouter.Delete("/v1/admin/external-endpoints/:key", DeleteManagedExternalEndpoint(cfg))
	protectedAPIRouter.Get("/v1/admin/monitors", GetAdminMonitors(cfg))
	protectedAPIRouter.Post("/v1/admin/monitors/batch", ExecuteBatchOperation(cfg))
	protectedAPIRouter.Get("/v1/admin/export", ExportManagedConfiguration(cfg))
	protectedAPIRouter.Post("/v1/admin/import", ImportManagedConfiguration(cfg))
