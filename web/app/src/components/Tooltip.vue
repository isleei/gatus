<template>
  ...
  >
    <div v-if="result" class="space-y-2">
      ...
      <!-- Timestamp -->
      <div>
        <div class="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{{ t('common.timestamp') }}</div>
      ...
      <div v-if="isSuiteResult && result.endpointResults">
        ...
        <div class="font-mono text-xs">
          <span :class="successCount === endpointCount ? 'text-green-500' : 'text-yellow-500'">
            {{ t('tooltip.passedCount', { success: successCount, total: endpointCount }) }}
          </span>
        </div>
        <!-- Endpoint breakdown -->
        <div v-if="result.endpointResults.length > 0" class="mt-1 space-y-0.5">
          ...
          <div v-if="result.endpointResults.length > 5" class="text-xs text-muted-foreground">
            {{ t('tooltip.andMore', { count: result.endpointResults.length - 5 }) }}
          </div>
        </div>
      </div>
      ...
      <div v-if="!isSuiteResult && result.conditionResults && result.conditionResults.length">
        <div class="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{{ t('common.conditions') }}</div>
      ...
      <div v-if="result.errors && result.errors.length">
        <div class="text-xs font-semibold text-muted-foreground uppercase tracking-wider">{{ t('common.errors') }}</div>
        <div class="font-mono text-xs space-y-0.5">
